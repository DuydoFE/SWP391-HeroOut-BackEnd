...
@RestController
@RequestMapping("/api/survey-results") // Base path remains the same
public class SurveyResultAPI { // Changed class name
    ...
    @PostMapping
    public ResponseEntity<SurveyResult> createSurveyResult(@RequestBody SurveyResultCreateRequest request) {
        Long currentAccountId = 1L;

        try {
            SurveyResult createdResult = surveyResultService.createSurveyResult(request, currentAccountId);
